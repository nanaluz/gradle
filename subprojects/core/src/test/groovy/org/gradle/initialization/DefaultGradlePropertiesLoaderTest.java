...
public class DefaultGradlePropertiesLoaderTest {
    ...
    public void mergeAddsPropertiesFromInstallationPropertiesFile() {
        writePropertyFile(gradleInstallationHomeDir, Cast.uncheckedCast(Cast.uncheckedCast(GUtil.map("settingsProp", "settings value"))));
    ...
    public void mergeAddsPropertiesFromUserPropertiesFile() {
        writePropertyFile(gradleUserHomeDir, Cast.uncheckedCast(GUtil.map("userProp", "user value")));
    ...
    public void mergeAddsPropertiesFromSettingsPropertiesFile() {
        writePropertyFile(settingsDir, Cast.uncheckedCast(GUtil.map("settingsProp", "settings value")));
    ...
    public void mergeAddsPropertiesFromEnvironmentVariablesWithPrefix() {
        envProperties = Cast.uncheckedCast(GUtil.map(
    ...
    public void mergeAddsPropertiesFromSystemPropertiesWithPrefix() {
        systemProperties = Cast.uncheckedCast(GUtil.map(
    ...
    public void mergeAddsPropertiesFromStartParameter() {
        startParameter.setProjectProperties(Cast.uncheckedCast(GUtil.map("paramProp", "param value")));
    ...
    public void projectPropertiesHavePrecedenceOverInstallationPropertiesFile() {
        writePropertyFile(gradleInstallationHomeDir, Cast.uncheckedCast(GUtil.map("prop", "settings value")));
        Map<String, String> projectProperties = Cast.uncheckedCast(GUtil.map("prop", "project value"));
    ...
    public void projectPropertiesHavePrecedenceOverSettingsPropertiesFile() {
        writePropertyFile(settingsDir, Cast.uncheckedCast(GUtil.map("prop", "settings value")));
        Map<String, String> projectProperties = Cast.uncheckedCast(GUtil.map("prop", "project value"));
    ...
    public void userPropertiesFileHasPrecedenceOverSettingsPropertiesFile() {
        writePropertyFile(gradleUserHomeDir, Cast.uncheckedCast(GUtil.map("prop", "user value")));
        writePropertyFile(settingsDir, Cast.uncheckedCast(GUtil.map("prop", "settings value")));
    ...
    public void environmentVariablesHavePrecedenceOverProjectProperties() {
        ...
        writePropertyFile(settingsDir, Cast.uncheckedCast(GUtil.map("prop", "settings value")));
        Map<String, String> projectProperties = Cast.uncheckedCast(GUtil.map("prop", "project value"));
        envProperties = Cast.uncheckedCast(GUtil.map(ENV_PROJECT_PROPERTIES_PREFIX + "prop", "env value"));
    ...
    public void systemPropertiesHavePrecedenceOverEnvironmentVariables() {
        ...
        Map<String, String> projectProperties = Cast.uncheckedCast(GUtil.map("prop", "project value"));
        envProperties = Cast.uncheckedCast(GUtil.map(ENV_PROJECT_PROPERTIES_PREFIX + "prop", "env value"));
        systemProperties = Cast.uncheckedCast(GUtil.map(SYSTEM_PROJECT_PROPERTIES_PREFIX + "prop", "system value"));
    ...
    public void startParameterPropertiesHavePrecedenceOverSystemProperties() {
        ...
        envProperties = Cast.uncheckedCast(GUtil.map(ENV_PROJECT_PROPERTIES_PREFIX + "prop", "env value"));
        systemProperties = Cast.uncheckedCast(GUtil.map(SYSTEM_PROJECT_PROPERTIES_PREFIX + "prop", "system value"));
        startParameter.setProjectProperties(Cast.uncheckedCast(GUtil.map("prop", "param value")));
    ...
    public void loadSetsSystemProperties() {
        ...
        writePropertyFile(gradleUserHomeDir, Cast.uncheckedCast(GUtil.map(SYSTEM_PROP_PREFIX + ".userSystemProp", "userSystemValue")));
        writePropertyFile(settingsDir, Cast.uncheckedCast(GUtil.map(
    ...
    public void reloadsProperties() {
        writePropertyFile(settingsDir, Cast.uncheckedCast(GUtil.map("prop1", "value", "prop2", "value")));

        File otherSettingsDir = tmpDir.createDir("otherSettingsDir");
        writePropertyFile(otherSettingsDir, Cast.uncheckedCast(GUtil.map("prop1", "otherValue")));
    ...
    public void startParameterSystemPropertiesHavePrecedenceOverPropertiesFiles() {
        writePropertyFile(gradleUserHomeDir, Cast.uncheckedCast(GUtil.map("systemProp.prop", "user value")));
        writePropertyFile(settingsDir, Cast.uncheckedCast(GUtil.map("systemProp.prop", "settings value")));
        systemProperties = Cast.uncheckedCast(GUtil.map("prop", "system value"));
